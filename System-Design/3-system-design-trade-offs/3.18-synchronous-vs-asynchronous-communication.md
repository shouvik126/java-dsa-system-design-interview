...
![Synchronous vs Asynchronous Communication Workflows](https://raw.githubusercontent.com/shouvik126/HLD_images/main/3.18-synchronous-vs-asynchronous-communication.png)
...
- **Cumulative Latency**: In a synchronous call chain where Service A calls B, which calls C, latency accumulates additively. Four microservices at **100 ms** each force a **400 ms** user-facing delay.
...
1. **"What if the message is delivered twice?"** → Answer with **Idempotency Keys** and deduplication tables.
2. **"What if the consumer keeps failing?"** → Answer with **Retries with Exponential Backoff** and routing persistent failures to a **Dead Letter Queue (DLQ)** with alerting.

If asked why not make everything asynchronous, cite the **availability multiplication math** (0.999⁵ ≈ 99.5%): every synchronous hop removed from the critical path removes its failure rate from your user-facing uptime number.
