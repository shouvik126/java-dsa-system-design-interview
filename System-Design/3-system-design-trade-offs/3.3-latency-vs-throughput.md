...
![Latency vs Throughput Time Axis Comparison](https://raw.githubusercontent.com/shouvik126/HLD_images/main/3.3-latency-vs-throughput-time-axis.png)
...
Memorizing this formula lets you calculate exact capacity requirements on the spot. For instance, to serve **2,000 QPS** with an average request latency of **50 ms** (**0.05 s**):
...
![Throughput vs Latency Utilization Curve](https://raw.githubusercontent.com/shouvik126/HLD_images/main/3.3-throughput-latency-headroom-curve.png)
...
![Latency Distribution Percentiles Long Tail](https://raw.githubusercontent.com/shouvik126/HLD_images/main/3.3-latency-distribution-percentiles.png)
...
Consequently, **roughly 63% of user page loads will experience at least one slow downstream call (> 1 s)**. Tail latency is not a rare edge case—in distributed architectures with fan-out queries, the tail becomes the typical user experience.
...
> *"With 1 million daily active users making 5 feed loads per day, average throughput is ~58 QPS, with peak throughput reaching 200–300 QPS. Applying Little's Law for a target p99 latency under 200 ms (**0.2 s**), we need **300 × 0.2 = 60** concurrent workers in flight. To maintain server utilization at ~60% headroom, we will size our fleet for 100 concurrent worker slots."*
...
- They are independent metrics linked by **Little's Law**: **Concurrency = Throughput × Latency**.
