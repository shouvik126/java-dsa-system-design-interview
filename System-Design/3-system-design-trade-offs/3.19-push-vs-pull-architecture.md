...
![Push vs Pull Architecture Fan-Out Diagram](https://raw.githubusercontent.com/shouvik126/HLD_images/main/3.19-push-vs-pull-architecture.png)
...
1. **"Where does the fan-out write happen?"** → Explain that fan-out tasks are pushed to an asynchronous task queue (e.g., Kafka/RabbitMQ worker pool) so author posts return immediately without blocking the user.
2. **"How do you handle consumer load?"** → Mention that pull-based workers provide built-in backpressure protection.
