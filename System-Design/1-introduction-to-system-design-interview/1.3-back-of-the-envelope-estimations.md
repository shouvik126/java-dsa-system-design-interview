...
1. **1 Day $\approx$ 100,000 Seconds**: A solar day actually contains 86,400 seconds. Rounding up to 100,000 simplifies all subsequent mental divisions. The ~14% error margin is negligible compared to other high-level assumptions.
2. **1 Million Requests/Day $\approx$ 12 Requests/Second (QPS)**: With this benchmark, calculating throughput becomes simple multiplication:
   - **1 Billion requests/day** $\approx$ **12,000 QPS**
   - **100 Million requests/day** $\approx$ **1,200 QPS**
...
3. **Utilize Powers of Ten ($10^N$)**: Keep magnitudes separate from base digits. Calculating $2 \times 10^9$ across multiple steps is much easier to track than managing `2,000,000,000`, making unit conversions obvious.
...
- **Replication**: 3x replication triples the footprint ($730\text{ PB} \times 3 \approx 2.2\text{ EB}$).
...
1. **RAM vs. Disk Seek ($\sim 100,000\times$ difference)**: In-memory access is roughly 100,000 times faster than disk I/O. This massive gap is why caching delivers the highest performance improvement in system design.
2. **SSD vs. HDD Seek ($\sim 100\times$ difference)**: Solid-State Drive (SSD) reads are roughly 100 times faster than mechanical spinning disk seeks. This ratio dictates database storage engine selection under heavy read operations.
3. **Cross-Ocean Network Round-Trip ($\sim 100\text{ ms}$)**: Transatlantic/transpacific network round-trips take ~100 ms due to the speed of light in fiber optic cables. No amount of software tuning can bypass physical distance, making multi-region deployments and edge CDNs essential for global users.
...
Declaring assumptions establishes a concrete baseline to design against and allows the interviewer to guide you. If scaling up by $10\times$ alters the architecture, state that explicitly:
...
- **Remember key anchor numbers**: 1 day $\approx$ 100,000 seconds; 1M requests/day $\approx$ 12 QPS.
