...
![Bloom Filter Using 3 Hash Functions](https://raw.githubusercontent.com/shouvik126/HLD_images/main/2.15-bloom-filter-3-hash-function.png)
...
2. **Adding an Element**:
   - Compute `k` indices by evaluating `k(item)`.
...
3. **Querying an Element**:
   ...
   - Inspect the bits at all `k` positions:
     - If at least one bit is `0` → Return **"Definitely Not Present"**.
     - If all `k` bits are `1` → Return **"Probably Present"**.
